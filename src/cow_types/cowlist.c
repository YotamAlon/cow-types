...
static PyObject *CowList_new(PyTypeObject *t, PyObject *Py_UNUSED(a),
...
static int CowList_init(CowListObject *self, PyObject *args,
...
static Py_ssize_t CowList_length(CowListObject *self)
{
    return PyList_GET_SIZE(self->data);
}
...
static int CowList_ass_subscript(CowListObject *self, PyObject *key,
                                 PyObject *value)
{
    PyObject *new_data =
        PyList_GetSlice(self->data, 0, PyList_GET_SIZE(self->data));
    if (!new_data) {
        return -1;
    }
    int r = value ? PyObject_SetItem(new_data, key, value)
...
static PyObject *CowList_iter(CowListObject *self)
{
    return PyObject_GetIter(self->data);
}

static PyObject *CowList_richcompare(CowListObject *self, PyObject *other,
                                     int op)
{
    PyObject *od = Py_IS_TYPE(other, &CowListType)
                       ? ((CowListObject *)other)->data
                       : other;
...
static PyObject *CowList_clear(CowListObject *self,
...
static PyObject *CowList_sort(CowListObject *self, PyObject *args,
                              PyObject *kwds)
{
    PyObject *new_data =
        PyList_GetSlice(self->data, 0, PyList_GET_SIZE(self->data));
...
static PyObject *CowList_reverse(CowListObject *self,
                                 PyObject *Py_UNUSED(ignored))
{
    PyObject *new_data =
        PyList_GetSlice(self->data, 0, PyList_GET_SIZE(self->data));
...
static PyObject *CowList_copy(CowListObject *self,
                              PyObject *Py_UNUSED(ignored))
{
    PyObject *copy =
        PyList_GetSlice(self->data, 0, PyList_GET_SIZE(self->data));
...
static PyObject *CowList_index(CowListObject *self, PyObject *args)
{
    ...
    for (Py_ssize_t i = start; i < stop; i++) {
        int eq = PyObject_RichCompareBool(PyList_GET_ITEM(self->data, i), item,
                                          Py_EQ);
        if (eq < 0) {
            return NULL;
        }
...
static PyGetSetDef CowList_getsetters[] = {{NULL}};
...
static PySequenceMethods CowList_as_sequence = {
    (lenfunc)CowList_length,      0, 0, (ssizeargfunc)CowList_item, 0, 0, 0,
...
static PyTypeObject CowListType = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "cow.cowlist",
